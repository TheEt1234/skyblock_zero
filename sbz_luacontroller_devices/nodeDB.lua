...
core.register_node('sbz_luacontroller_devices:node_db', {
    description = 'Node DB',
    ...
    on_logic_send = function(pos, msg, from_pos)
        ...
        if msg.type == 'search' then
            if
                not libox.type_check(msg, {
                    type = libox.type('string'),
                    text = libox.type('string'),
                    give_basic_def = function(x)
                        return libox.type('boolean')(x) or x == nil
                    end,
                    max_results = function(x)
                        return libox.type('number') or x == nil
                    end,
                    exclude_groups = function(x)
                        return libox.type('table')(x) or x == nil
                    end,
                })
            then
                return
            ...
            if string.sub(msg.text, 1, #'group:') == 'group:' then
                ...
                for k, v in pairs(core.registered_items) do
                    ...
                    for kk, vv in pairs(groups) do
                        if core.get_item_group(k, vv) == 0 then fit = false end
                    end
                    if fit == true then result[#result + 1] = k end
                end
            else
                for k, v in pairs(core.registered_items) do
                    if
                        string.find(k, msg.text, 0, true)
                        or string.find(v.short_description or v.description or '', msg.text, 0, true)
                    then
                        result[#result + 1] = k
                    end
                end
            ...
            if msg.exclude_groups then
                ...
                for k, v in ipairs(result) do
                    ...
                    if fit then new_result[#new_result + 1] = v end
                end
                result = new_result
            end
                ...
                for k, v in pairs(result) do
                    ...
                    result[k] = {
                        ...
                        image = item.inventory_image
                            or item.wield_image
                            or (
                                type(item.tiles) == 'table'
                                and core.inventorycube(
                                    item.tiles[1] or '',
                                    item.tiles[3] or item.tiles[1] or '',
                                    item.tiles[5] or item.tiles[1] or ''
                                )
                            ),
                    }
                end
...
do -- Node DB recipe scope
    local Node_DB = 'sbz_luacontroller_devices:node_db'
