...
local function get_index(inv, item)
    ...
    for k, v in ipairs(list) do
        ...
        if item:equals(v) and item:is_known() then return k end
    end
    return false
end
...
local function build(pos, owner, def, param2, inv, index, dir)
    if param2 ~= nil then
        if param2 < 0 or param2 > 255 then param2 = nil end
    end
...
local function dont_wear_out(stack, old_stack, player, item_def)
    if stack:get_name() == old_stack:get_name() then
        if
            stack:get_wear() ~= old_stack:get_wear()
            and stack:get_count() == old_stack:get_count()
...
local function see(pos, lc_from_pos, builder_from_pos)
    ...
    result.inventory = sbz_luacs.kill_itemstacks(meta.inventory)

    sbz_luacs.send_luacomm_message(lc_from_pos, result, builder_from_pos)
end
...
sbz_api.register_machine('sbz_luacontroller_devices:builder', {
    ...
    action_subtick = function(pos, _, meta, supply, demand)
        ...
        local queue_can_handle =
            math.min(math.min(queue_max, #queued_events), math.max(0, math.floor(net / (queue_max * power_per_action)))) -- spagetti math
        ...
        local function inner_loop(i)
            ...
            local ok = libox.type_check(e, {
                ...
                pos2 = function(x)
                    return x == nil or libox.type_vector(x)
                end,
                item = function(x)
                    return x == nil or libox.type('string')(x)
                end,
                param2 = function(x)
                    return x == nil and true or libox.type('number')(x)
                end,
                from_pos = libox.type_vector,
                dir = function(x)
                    if valid_dirs[x] or x == nil then
                        return true
                    else
                        return false
                    end
...
unified_inventory.register_craft {
    type = 'ele_fab',
    output = 'sbz_luacontroller_devices:builder',
    items = {
        'pipeworks:puncher 4',
...
mesecon.register_on_mvps_move(function(moved_nodes)
    for i = 1, #moved_nodes do
        ...
        if builder_queues[core.hash_node_position(moved_node.oldpos)] then
            builder_queues[core.hash_node_position(moved_node.pos)] =
                builder_queues[core.hash_node_position(moved_node.oldpos)]
