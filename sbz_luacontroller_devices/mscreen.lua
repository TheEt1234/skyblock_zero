...
local function remove_entity(pos)
    ...
    for _, i in pairs(entitiesNearby) do
        if i:get_luaentity() and i:get_luaentity().name == 'sbz_luacontroller_devices:matrix_screen_entity' then
            i:remove()
        end
    end
end
...
local function generate_texture(pos, serdata)
    ...
    if type(data) ~= 'table' then
        core.log('error', '[sbz_logic_devices] Failed to deserialize display data at ' .. core.pos_to_string(pos, 0))
        return
    end
    local bincolors = {}
    ...
    for y = 1, size do
        ...
        for x = 1, size do
            bincolors[index] = data[y][x] or core.colorspec_to_bytes('black') -- ive optimized it a lil to not be a warcrime
            index = index + 1
        end
    end
...
local function update_display(pos)
    ...
    local entity = core.add_entity(pos, 'sbz_luacontroller_devices:matrix_screen_entity')
...
core.register_entity('sbz_luacontroller_devices:matrix_screen_entity', {
    initial_properties = {
...
core.register_node('sbz_luacontroller_devices:matrix_screen', {
    ...
    on_punch = function(screenpos, _, player)
        ...
        repeat
            ...
            if pointed and pointed.type == 'node' then
                ...
                if node.name == 'sbz_luacontroller_devices:matrix_screen' then
    ...
    on_logic_send = function(pos, msg, from_pos)
        ...
        for y = 1, size do
            ...
            for x = 1, size do
                ...
                if bytes == nil then bytes = core.colorspec_to_bytes('black') end
                data[y][x] = bytes
            end
        end
...
core.register_lbm({
    name = 'sbz_luacontroller_devices:matrix_screen_respawn',
    label = 'Respawn matrix screen entities',
    nodenames = { 'sbz_luacontroller_devices:matrix_screen' },
...
unified_inventory.register_craft {
    ...
    output = 'sbz_luacontroller_devices:matrix_screen',
    width = 2,
    height = 2,
}
