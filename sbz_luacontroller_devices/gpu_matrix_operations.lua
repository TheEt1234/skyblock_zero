...
local function matrix_transform_3x3(mat, x, y)
    ...
    return (mat[1][1] * x + mat[1][2] * y + mat[1][3]) / z, (mat[2][1] * x + mat[2][2] * y + mat[2][3]) / z
end

local rounding_function = math.round

local default_color = core.colorspec_to_bytes('black'):sub(1, 3)
--m* = max *
-- e.g.: mx1 = max x1
...
local function transform_buffer(
    ...
    for k, v in ipairs(transformations) do
        ...
        if not (nx < mx1 or nx > mx2 or ny < my1 or ny > my2) then
    ...
    for y = y1, y2 do
        for x = x1, x2 do
            ...
            buffer_modifications[#buffer_modifications + 1] = {
                (y - 1) * buffer_sizex + x,
                string.char(r) .. string.char(g) .. string.char(b),
            }
        end
    end
