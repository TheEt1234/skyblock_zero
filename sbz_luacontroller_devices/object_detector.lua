...
core.register_node('sbz_luacontroller_devices:object_detector', {
    ...
    on_logic_send = function(pos, msg, from_pos)
        ...
        if type(msg) == 'table' then
            if type(msg.inventories) == 'boolean' then settings.inventories = msg.inventories end
            if type(msg.props) == 'boolean' then settings.props = msg.props end
        end
...
do -- Object Detector recipe scope
    local Object_Detector = 'sbz_luacontroller_devices:object_detector'
