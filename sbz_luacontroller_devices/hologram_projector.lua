...
local function all_in_range(t, min, max)
    for k, v in pairs(t) do
        if type(v) == 'number' then
            if math.abs(v) < min or math.abs(v) > max and not core.is_nan(v) then return false end
        end
    end
    return true
end
                    ...
                    if v.animation.type == 'vertical_frames' then
                        if
                            libox.type_check(v.animation, {
                                ...
                                length = function(x)
                                    return type(x) == 'number' and x > 0.1
                                end,
                            }) == false
                        then
                            v.animation = nil
...
local function validate_object_properties(props, not_strict)
    ...
    local t = {
        ...
        automatic_face_movement_dir = function(x)
            return x == nil or x == false or type(x) == 'number'
        end,
        ...
        infotext = ton 'string',
        static_save = function(x)
            return x == false
        end,
        damage_texture_modifier = function(x)
    ...
    props.damage_texture_modifier = props.damage_texture_modifier
        and transform_texture_name(props.damage_texture_modifier, true)
    return ok, errmsg
end

core.register_entity('sbz_luacontroller_devices:hologram', {
    ...
    on_step = function(self, dtime, moveresult)
        ...
        if parent_node.name ~= 'sbz_luacontroller_devices:hologram_projector' then
            self.object:remove()
            return
        end

        local diff = vector.subtract(parent, self.object:get_pos())
        if math.abs(diff.x) > range_max or math.abs(diff.y) > range_max or math.abs(diff.z) > range_max then
            local meta = core.get_meta(parent)
            local subscribed = vector.from_string(meta:get_string('subscribed'))
            sbz_logic.send(subscribed, {
                type = 'walked_off',
...
local function exec_command(pos, cmd, from_pos)
    ...
    for k, v in pairs(stuff.objects) do
        if v:is_valid() == false then stuff.objects[k] = nil end
    end
    ...
    if cmd.type == 'object' then
        ...
        local obj = core.add_entity(
            cmd.pos,
            'sbz_luacontroller_devices:hologram',
            core.serialize({
                parent = pos,
                properties = cmd,
            })
    ...
    elseif cmd.type == 'remove' and type(cmd.id) == 'string' then
        if stuff.objects[cmd.id] then
            if stuff.objects[cmd.id].remove then stuff.objects[cmd.id]:remove() end
            stuff.objects[cmd.id] = nil
        else
            notify {
                type = 'error',
    ...
    elseif cmd.type == 'modify_object' then -- set acceleration n stuff like that... this wont be painful at all xD
        ...
        if stuff.objects[id] == nil then
            return notify { type = 'error', msg = 'Invalid id in modify_object command' }
        end
        ...
        if cmd.set_properties then
            ...
            if not ok then
                notify {
                    type = 'error',
                    msg = 'Invalid properties in modify_object set_properties table, problematic element: '
                        .. invalid_element,
                }
            end
        ...
        if cmd.set_nametag_attributes then
            ...
            if not ok then
                notify {
                    type = 'error',
                    msg = 'Invalid properties in modify_object set_nametag_attributes table, problematic element: '
                        .. invalid_element,
                }
            end
        ...
        if libox.type_vector(cmd.velocity) then obj:set_velocity(cmd.velocity) end

        if libox.type_vector(cmd.rotation) then obj:set_rotation(cmd.rotation) end

        if libox.type_vector(cmd.acceleration) then obj:set_acceleration(cmd.acceleration) end
        ...
        if type(cmd.bone_override) == 'table' then
            if
                libox.type_check(cmd.bone_override, {
...
core.register_node('sbz_luacontroller_devices:hologram_projector', {
...
unified_inventory.register_craft {
    ...
    items = {
        ...
        'sbz_luacontroller_devices:matrix_screen 15',
        'sbz_luacontroller_devices:object_detector',
        'unifieddyes:colorium_blob 10',
    },
    output = 'sbz_luacontroller_devices:hologram_projector',
    width = 2,
    height = 2,
}
