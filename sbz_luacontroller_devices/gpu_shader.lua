...
local function apply_shader(buffer, shader, pos, from_pos)
    ...
    for thread = 1, threads do
        core.handle_async(function(buffer, shader, thread, max_threads)
            ...
            if not f then return nil, nil, false, errmsg end
            ...
            for y = ystart, math.min(ysize, ystart + segment) do
                for x = 1, xsize do
                    ...
                    if ok == false or type(result) ~= 'table' then
                        return nil, core.get_us_time() - t0, false, result
                    end

                    if type(result) ~= 'table' then return nil, nil, false end
                    local r, g, b = result.r, result.g, result.b
                    if type(r) ~= 'number' or core.is_nan(r) then
                        return nil, nil, false, 'return format must be in { r=int,g=int,b=int }'
                    end
                    if type(g) ~= 'number' or core.is_nan(g) then
                        return nil, nil, false, 'return format must be in { r=int,g=int,b=int }'
                    end
                    if type(b) ~= 'number' or core.is_nan(b) then
                        return nil, nil, false, 'return format must be in { r=int,g=int,b=int }'
                    end
            ...
            return new_buffer, core.get_us_time() - t0, true
        end, function(new_buffer, lag, success, error)
            ...
            for i = 1, threads do
                if not thread_data[i].finished then return end
            end
            ...
            for i = 1, threads do
                if thread_data[i].success == false then all_successful = false end
            end
            ...
            if node.name ~= 'sbz_luacontroller_devices:gpu' then return end
