...
sbz_api.register_machine('sbz_luacontroller_devices:luanium_attractor', {
    ...
    action_subtick = function(pos, node, meta, supply, demand)
        ...
        if attract == 0 then
            ...
            if supply > demand + math.abs((attract * 10)) then
                meta:set_string('infotext', 'Working, power use: ' .. math.abs(attract * 10) * 4 .. 'Cj')
...
do -- Luanium Attractor recipe scope
    local Luanium_Attractor = 'sbz_luacontroller_devices:luanium_attractor'
