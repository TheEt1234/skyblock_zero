...
local bresenham, xiaolin_wu, midpoint_circle =
    loadfile(core.get_modpath('sbz_logic_devices') .. '/gpu_line_algos.lua')()
...
local function after_place_node(pos, placer)
    ...
    local nodes = core.find_nodes_in_area(
        vector.subtract(pos, area_vec),
        vector.add(pos, area_vec),
        'sbz_luacontroller_devices:gpu'
    )

    if #nodes > 1 then
        core.chat_send_player(
            placer:get_player_name(),
            ("2 Gpus can't be near eachother, they need to be %s blocks apart, or you have to have the place_gpus_unlimited privledge"):format(
                a
            )
        )
...
local function flood_fill(buffers, command)
    ...
    local function similar(target_color, compare_color, tolerance)
        return (math.abs(target_color.r - compare_color.r) <= tolerance)
            and (math.abs(target_color.g - compare_color.g) <= tolerance)
            and (math.abs(target_color.b - compare_color.b) <= tolerance)
    end
    ...
    while #stack ~= 0 do
        ...
        if not real_buffer[idx(pos.x, pos.y)] then return end
        ...
        if similar(col, checking_color, tolerance) then
            ...
            for k, v in ipairs(ruleset_4dir) do
                ...
                if (nx <= b.xsize) and (ny <= b.ysize) and (ny > 0) and (nx > 0) and not seen[hash(nx, ny)] then
...
local commands = {
    ...
    ['send'] = {
        ...
        f = function(buffers, command, pos, from_pos)
            if not libox.type_vector(command.to_pos) then command.to_pos = from_pos end
            local b = buffers[command.index]
            if b ~= nil then
    ...
    ['copy'] = {
        ...
        f = function(buffers, command)
            ...
            local sx1, sy1, sx2, sy2 = validate_area(
                src_buf,
                command.srcx,
                command.srcy,
            ...
            local dx1, dy1, dx2, dy2 = validate_area(
                dst_buf,
                command.dstx,
                command.dsty,
            ...
            if (sx1 == nil) or (dx1 == nil) then return end
            local px1, px2
    ...
    ['send_packed'] = {
        ...
        f = function(buffers, command, pos, from_pos)
            ...
            if command.base64 then result = core.encode_base64(result) end
            sbz_logic.send_l(command.to_pos, result, from_pos) -- send as if logic sent it
        end,
    },
    ...
    ['load_packed'] = {
        ...
        f = function(buffers, command)
            ...
            if command.base64 then data = core.decode_base64(command.data) end
            ...
            for y = y1, y2 do
                for x = x1, x2 do
                    ...
                    if packed_data and #packed_data == 3 then real_buf[(y - 1) * b.xsize + x] = packed_data end
                    idx = idx + 3
                end
            end
        end,
    ...
    ['circle'] = {
        ...
        f = function(buffers, command)
            ...
            if command.hollow == false then
                for yi = -r, r do
                    for xi = -r, r do
                        if yi ^ 2 + xi ^ 2 <= r_squared then real_buffer[(y + yi - 1) * b.xsize + x + xi] = color end
                    end
                end
            else
                ...
                for chary = 1, #char do
                    ...
                    for charx = 1, xsize do
                        ...
                        if px == true then
                            ...
                            if b.buffer[target_idx] ~= nil then b.buffer[target_idx] = color end
                        end
                    end
                end
            end
    ...
    ['transform'] = {
        ...
        f = function(buffers, command)
            ...
            if
                libox.type_check(command.matrix, {
                    [1] = {
                        [1] = libox.type('number'),
                        [2] = libox.type('number'),
                    },
            ...
            transform_buffer(
                ...
                mx2,
                my2,
                transform_color(command.transparent_color),
                command.origin_x,
                command.origin_y,
                is_2x2_matrix
            )
...
core.register_node('sbz_luacontroller_devices:gpu', {
    description = 'GPU',
    ...
    on_logic_send = function(pos, msg, from_pos)
        ...
        if last_measured ~= os.time() then lag = 0 end
...
unified_inventory.register_craft {
    ...
    output = 'sbz_luacontroller_devices:gpu',
    width = 2,
    height = 2,
}
