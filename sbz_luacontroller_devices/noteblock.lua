...
local t = type
core.register_node('sbz_luacontroller_devices:noteblock', {
    ...
    on_logic_send = function(pos, msg, from_pos)
        ...
        for i = 1, math.min(#msg, MAX_COMMANDS_AT_ONCE) do
            ...
            if type(cmd) == 'string' then
                core.sound_play(cmd, { pos = pos })
            elseif type(cmd) == 'table' then
                if t(cmd.sound) ~= 'string' then break end

                for _, i in ipairs({ 'pitch', 'speed', 'volume', 'gain' }) do
                    if t(cmd[i]) == 'string' then cmd[i] = tonumber(cmd[i]) end
                end

                if not cmd.volume then cmd.volume = cmd.gain end
                local volume = 1
                if t(cmd.volume) == 'number' then volume = math.max(0, math.min(1, cmd.volume)) end
                if not cmd.pitch then cmd.pitch = cmd.speed end
                local pitch = 1
                if t(cmd.pitch) == 'number' then pitch = math.max(0.05, math.min(10, cmd.pitch)) end
                ...
                if t(cmd.cut) == 'number' and cmd.cut >= 0 then
                    ...
                    local handle = core.sound_play(sound_spec, sound_param, false)
                    core.after(cmd.cut, core.sound_stop, handle)
                elseif
                    t(cmd.fadestep) == 'number'
                    and t(cmd.fadegain) == 'number'
                    and cmd.fadegain >= 0
                    and type(cmd.fadestart) == 'number'
                    and cmd.fadestart >= 0
                then
                    local handle = core.sound_play(sound_spec, sound_param, false)
                    core.after(cmd.fadestart, core.sound_fade, handle, cmd.fadestep, cmd.fadegain)
                else
                    core.sound_play(sound_spec, sound_param, true)
                end
            end
        end
    end,
...
unified_inventory.register_craft {
    ...
    output = 'sbz_luacontroller_devices:noteblock',
    width = 2,
    height = 2,
}
